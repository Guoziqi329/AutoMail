...
class App(ctk.CTk):
    def __init__(self):
        ...
        # 侧边栏标题
        ctk.CTkLabel(self.sidebar_frame, text="自动邮件", height=95, font=ctk.CTkFont(size=20, weight="bold")).grid(row=0, column=0, padx=20, pady=(20, 10))

        # 侧边栏按钮
        ctk.CTkButton(self.sidebar_frame, text="主界面", command=self.show_home, width=150).grid(row=1, column=0, padx=25, pady=10)
        ctk.CTkButton(self.sidebar_frame, text="邮件服务设置", command=self.show_page1, width=150).grid(row=2, column=0, padx=25, pady=10)
        ctk.CTkButton(self.sidebar_frame, text="数据库服务设置", command=self.show_page2, width=150).grid(row=3, column=0, padx=25, pady=10)
        ctk.CTkButton(self.sidebar_frame, text="数据库表、字段设置", command=self.show_page3, width=150).grid(row=4, column=0, padx=25, pady=10)
        ctk.CTkButton(self.sidebar_frame, text="日志设置", command=self.show_page4, width=150).grid(row=5, column=0, padx=25, pady=10)
    ...
    def show_home(self):
        ...
        ctk.CTkLabel(self.current_frame, text="欢迎使用自动邮件", font=ctk.CTkFont(size=25, weight="bold")).grid(row=0, column=0, columnspan=5, padx=40, pady=30)

        # 添加按钮
        ctk.CTkButton(self.current_frame, text="开始", font=ctk.CTkFont(size=15), command=self.email_detection).grid(row=1, column=0, columnspan=3, padx=50, pady=50)
        ...
        ctk.CTkCheckBox(checkbox_textbox_frame, text="允许循环", font=ctk.CTkFont(size=15), variable=self.checkbox_var, command=self.toggle_textbox_state).grid(row=0, column=0, columnspan=3, padx=30, pady=(15, 5))

        # 添加“each”标签
        ctk.CTkLabel(checkbox_textbox_frame, text="每", font=ctk.CTkFont(size=15)).grid(row=1, column=0, padx=(20, 5), pady=(10, 15))
        ...
        # 添加“minutes”标签
        ctk.CTkLabel(checkbox_textbox_frame, text="分钟", font=ctk.CTkFont(size=15)).grid(row=1, column=2, padx=(5, 20), pady=(10, 15))
    ...
    def show_page1(self):
        ...
        welcome_label = ctk.CTkLabel(self.current_frame, text="邮件服务设置",
                                     font=ctk.CTkFont(size=25, weight="bold"))
        welcome_label.grid(row=0, column=0, columnspan=5, padx=40, pady=30, sticky="w")

        # 创建文本框
        self.updateData = list()
        self.page1_and_page2_frame(1, 0, "邮件地址", "请输入邮件地址", "email_address")
        self.page1_and_page2_frame(2, 0, "邮件密码", "请输入邮件密码", "email_password")
        self.page1_and_page2_frame(3, 0, "邮件IMAP服务地址", "请输入邮件IMAP服务器地址", "email_host")
        self.page1_and_page2_frame(4, 0, "邮件IMAP服务端口号", "请输入IMAP服务器端口号", "email_port")
        self.page1_and_page2_frame(5, 0, "发件人条件", "请输入发件人条件", "sender_email")

        # 添加按钮
        button = ctk.CTkButton(self.current_frame, text="保存", command=self.get_email_data)
    ...
    def show_page2(self):
        ...
        welcome_label = ctk.CTkLabel(self.current_frame, text="数据库服务设置",
                                     font=ctk.CTkFont(size=25, weight="bold"))
        welcome_label.grid(row=0, column=0, columnspan=5, padx=40, pady=30, sticky="w")

        self.updateData = list()
        self.page1_and_page2_frame(1, 0, "数据库用户", "请输入数据库用户", "database_user")
        self.page1_and_page2_frame(2, 0, "数据库密码", "请输入数据库密码", 'database_password')
        self.page1_and_page2_frame(3, 0, "数据库地址", "请输入数据库地址", 'database_host')
        self.page1_and_page2_frame(4, 0, "数据库端口号", "请输入数据库端口号", "database_port")
        self.page1_and_page2_frame(5, 0, "数据库实例名", "请输入数据库实例名", "database_server_name")

        # 添加按钮
        update_button = ctk.CTkButton(self.current_frame, text="保存", command=self.get_database_data)
    ...
    def show_page3(self):
        ...
        ctk.CTkLabel(self.current_frame, text="数据库表、字段设置", font=ctk.CTkFont(size=25, weight="bold")).grid(row=0, column=0, columnspan=5, padx=40, pady=30, sticky="w")
        ...
        # 添加断电账号表标签
        ctk.CTkLabel(frame_of_power_outage_account, text="断电账号表", font=ctk.CTkFont(size=17), height=30, width=250).grid(row=0, column=0, columnspan=4, padx=150, pady=3)

        # 添加断电账号表名标签和文本框
        self.page3_label_and_entry(frame_of_power_outage_account, 1, "表名称", "请输入断电账号表名", "power_off_table_name")

        # 添加时间字段标签和文本框
        self.page3_label_and_entry(frame_of_power_outage_account, 2, "时间字段", "请输入时间字段字段名称", "power_off_time_column")

        # 添加账号字段标签和文本框
        self.page3_label_and_entry(frame_of_power_outage_account, 3, "账号字段", "请输入账号字段字段名称", "power_off_account_column")
        ...
        # 添加邮件管理表标签
        ctk.CTkLabel(frame_of_email_management, text="邮件管理表", font=ctk.CTkFont(size=17), height=30, width=250).grid(row=0, column=0, columnspan=4, padx=150, pady=3)

        # 添加邮件管理表的标签和文本框
        self.page3_label_and_entry(frame_of_email_management, 1, "表名称", "请输入邮件管理表表名称", "email_management_table_name")

        # 添加emailId字段的标签和文本框
        self.page3_label_and_entry(frame_of_email_management, 2, "邮件ID字段", "请输入邮件ID字段字段名称", "email_management_email_id_column")

        # 添加received from字段的标签和文本框
        self.page3_label_and_entry(frame_of_email_management, 3, "发件人字段", "请输入发件人字段字段名称", "email_management_receive_from_column")

        # 添加received time字段的标签和文本框
        self.page3_label_and_entry(frame_of_email_management, 4, "接收时间字段", "请输入接收时间字段字段名称", "email_management_receive_time_column")

        update_button = ctk.CTkButton(self.current_frame, text="保存", command=self.update_database_fields)
    ...
    def show_page4(self):
        ...
        welcome_label = ctk.CTkLabel(self.current_frame, text="日志设置",
                                     font=ctk.CTkFont(size=25, weight="bold"))
        welcome_label.grid(row=0, column=0, columnspan=5, padx=40, pady=30, sticky="w")
        ...
        self.show_logging_dir_info(logging_information_frame, 0,  "Error 日志文件", self.set_data["log_path"], "error")
        self.show_logging_dir_info(logging_information_frame, 1,  "Warning 日志文件", self.set_data["log_path"], "warning")
        self.show_logging_dir_info(logging_information_frame, 2,  "Info 日志文件", self.set_data["log_path"], "info")

        update_button = ctk.CTkButton(self.current_frame, text="保存", command=self.folder_update)
        update_button.grid(row=6, column=0, columnspan=6, padx=20, pady=25)
